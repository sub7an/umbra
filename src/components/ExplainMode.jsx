  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        padding: '8px 12px', borderBottom: '1px solid rgba(245,158,11,0.08)',
        background: 'rgba(245,158,11,0.04)',
      }}>
        <span style={{ display:'inline-block', width:6, height:6, borderRadius:'50%', background:'#f59e0b', boxShadow:'0 0 6px #f59e0b', animation: loading ? 'umbra-pulse 0.8s ease-in-out infinite' : 'none', flexShrink:0 }} />
        <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:8, letterSpacing:'0.18em', color:'rgba(245,158,11,0.5)', flex:1 }}>
          {loading ? 'ANALYZING…' : 'UMBRA EXPLAINS'}
        </span>
        <button
          onClick={onClose}
          style={{ background:'none', border:'none', cursor:'pointer', color:'rgba(245,158,11,0.3)', fontSize:14, lineHeight:1, padding:0 }}
        >×</button>
      </div>
      {/* Body */}
      <div style={{ padding: '12px' }}>
        {loading ? (
          <div style={{ display:'flex', gap:4 }}>
            {[0,1,2].map(i => (
              <span key={i} style={{ display:'inline-block', width:4, height:4, borderRadius:'50%', background:'rgba(245,158,11,0.4)', animation:`umbra-pulse 1s ease-in-out ${i*0.18}s infinite` }} />
            ))}
          </div>
        ) : (
  ...
  return (
    <div style={{
      ...
      <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
        <circle cx="14" cy="14" r="5" stroke="#f59e0b" strokeWidth="1" opacity="0.7"/>
        <line x1="14" y1="0" x2="14" y2="8"  stroke="#f59e0b" strokeWidth="1" opacity="0.7"/>
        <line x1="14" y1="20" x2="14" y2="28" stroke="#f59e0b" strokeWidth="1" opacity="0.7"/>
        <line x1="0" y1="14" x2="8"  y2="14" stroke="#f59e0b" strokeWidth="1" opacity="0.7"/>
        <line x1="20" y1="14" x2="28" y2="14" stroke="#f59e0b" strokeWidth="1" opacity="0.7"/>
      </svg>
    </div>
  )
}
