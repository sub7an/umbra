...
export default function PhysicsTutor() {
  ...
  return (
    <div style={{ borderTop: '1px solid rgba(180,77,255,0.07)', background: 'rgba(1,6,12,0.97)', flexShrink: 0, display: 'flex', flexDirection: 'column-reverse' }}>
      <style>{`
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <svg width="11" height="11" viewBox="0 0 12 12" fill="none"
            ...
            <path d="M6 1L10.33 3.5V8.5L6 11L1.67 8.5V3.5L6 1Z"
              stroke={open ? `rgba(${glowRGB},0.9)` : 'rgba(180,77,255,0.35)'}
              strokeWidth="1"
          ...
          <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:8, letterSpacing:'.22em',
            textTransform:'uppercase', color: open ? '#b44dff' : 'rgba(180,77,255,0.38)', transition:'color .15s' }}>
            UMBRA AI
          </span>
          {streaming && (
            <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:7, letterSpacing:'.15em',
              color:'#b44dff', animation:'umbra-pulse 0.8s ease-in-out infinite' }}>THINKING</span>
          )}
          {isDreaming && (
            <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:7, letterSpacing:'.15em',
          ...
          {challenge?.won && (
            <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:7, letterSpacing:'.15em', color:'rgba(180,77,255,0.6)' }}>SOLVED</span>
          )}
          {messages.length > 0 && !streaming && !isDreaming && !challenge && (
            <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:7, letterSpacing:'.12em', color:'rgba(180,77,255,0.3)' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <span
            ...
            style={{ fontSize:10, cursor:'pointer', userSelect:'none',
              color: voiceOn ? '#b44dff' : 'rgba(180,77,255,0.2)', transition:'color .15s' }}
          >{voiceOn ? '🔊' : '🔇'}</span>
          <span style={{ fontSize:8, color:'rgba(180,77,255,0.25)', fontFamily:'monospace',
        ...
        <div style={{ display:'flex', flexDirection:'column', maxHeight:380 }}>
          ...
          {insight && (
            <div style={{ margin:'0 10px 6px', padding:'8px 10px',
              background:'rgba(180,77,255,0.03)', borderRadius:2,
              borderLeft:'2px solid rgba(180,77,255,0.4)',
              border:'1px solid rgba(180,77,255,0.1)',
              animation:'umbra-fade-in 0.5s ease' }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:7,
                  letterSpacing:'.2em', textTransform:'uppercase', color:'rgba(180,77,255,0.45)' }}>
                  ◈ CONNECTION DETECTED
                </span>
                <button onClick={() => setInsight(null)} style={{ fontFamily:'JetBrains Mono, monospace',
                  fontSize:7, color:'rgba(180,77,255,0.25)', background:'none', border:'none', cursor:'pointer', padding:0 }}>✕</button>
              </div>
          ...
          {challenge && (
            <div style={{ margin:'0 10px 6px', padding:'8px 10px',
              ...
              borderLeft:`2px solid ${challenge.won ? 'rgba(180,77,255,0.5)' : 'rgba(251,191,36,0.5)'}`,
              border:`1px solid ${challenge.won ? 'rgba(180,77,255,0.12)' : 'rgba(251,191,36,0.12)'}` }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:3 }}>
                <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:7, letterSpacing:'.18em',
                  textTransform:'uppercase', color: challenge.won ? 'rgba(180,77,255,0.7)' : 'rgba(251,191,36,0.7)' }}>
              ...
              <p style={{ fontFamily:'JetBrains Mono, monospace', fontSize:8,
                color: challenge.won ? 'rgba(180,77,255,0.5)' : 'rgba(251,191,36,0.4)', marginTop:3 }}>
          ...
          <div style={{ flex:1, overflowY:'auto', padding:'4px 14px 10px',
            ...
            scrollbarWidth:'thin', scrollbarColor:'rgba(180,77,255,0.12) transparent' }}>

            {/* Starters */}
            {messages.length === 0 && !streaming && (
              <div style={{ paddingTop:4 }}>
                <p style={{ fontFamily:'JetBrains Mono, monospace', fontSize:7, letterSpacing:'.2em',
                  textTransform:'uppercase', color:'rgba(180,77,255,0.28)', marginBottom:8 }}>TRY ASKING</p>
                {starters.map((q, i) => (
                  <button key={i} onClick={() => send(q)} style={{
                    display:'block', width:'100%', textAlign:'left',
                    fontFamily:'JetBrains Mono, monospace', fontSize:10,
                    color:'rgba(180,77,255,0.5)', background:'none', border:'none',
                    cursor:'pointer', padding:'5px 0', borderBottom:'1px solid rgba(180,77,255,0.05)',
                    transition:'color .1s',
                  }}
                  onMouseEnter={e => e.currentTarget.style.color='#b44dff'}
                  onMouseLeave={e => e.currentTarget.style.color='rgba(180,77,255,0.5)'}
                  >→ {q}</button>
                ))}
                {CHALLENGES[moduleId] && (
                ...
                <div key={i} style={{ paddingLeft:10, borderLeft:`2px solid ${borderColor}`,
                    ...
                    stepsToShow.map((step, si) => (
                      <div key={si} style={{ marginBottom: si < stepsToShow.length-1 ? 8 : 0,
                        ...
                        <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:9,
                          color:'rgba(180,77,255,0.38)', marginRight:6 }}>§{si+1}</span>
                        {step}
                      </div>
                    ))
                  ) : (
                  ...
                  {m.derivation && isLastDeriv && derivVisible < (m.steps?.length ?? 0) && (
                    <span style={{ display:'inline-block', width:6, height:11,
                      background:'#b44dff', marginLeft:2, verticalAlign:'text-bottom',
            ...
            {/* Streaming */}
            {streamText && (
              <div style={{ paddingLeft:10, borderLeft:'2px solid rgba(180,77,255,0.35)',
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6,
            padding:'8px 14px', borderTop:'1px solid rgba(180,77,255,0.07)' }}>
            <input
              ref={inputRef}
              value={input}
              ...
              style={{ flex:1, background:'rgba(180,77,255,0.04)',
                border:'1px solid rgba(180,77,255,0.10)', borderRadius:2, padding:'6px 10px',
                fontFamily:'JetBrains Mono, monospace', fontSize:10,
                color:'rgba(220,240,235,0.85)', outline:'none', transition:'border-color .15s' }}
              onFocus={e => e.target.style.borderColor='rgba(180,77,255,0.30)'}
              onBlur={e  => e.target.style.borderColor='rgba(180,77,255,0.10)'}
            />
            ...
            <button onClick={() => send(input)}
              ...
              style={{ fontFamily:'JetBrains Mono, monospace', fontSize:8, letterSpacing:'.15em',
                ...
                border:'1px solid rgba(180,77,255,0.18)', background:'rgba(180,77,255,0.05)',
                color:(input.trim() && !streaming) ? '#b44dff' : 'rgba(180,77,255,0.2)',
          ...
          <div style={{ padding:'4px 14px 7px', display:'flex', justifyContent:'space-between' }}>
            <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:7,
              letterSpacing:'.12em', color:'rgba(180,77,255,0.18)' }}>
              / to focus · Enter to send · {moduleId.replace(/-/g,' ')}
            </span>
            {journal.visited.length > 1 && (
              <span style={{ fontFamily:'JetBrains Mono, monospace', fontSize:7,
                letterSpacing:'.1em', color:'rgba(180,77,255,0.22)' }}>
