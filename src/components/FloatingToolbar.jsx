...
function Btn({ label, icon, onClick, active, color = '#b44dff', title }) {
  const base = `rgba(${color === '#b44dff' ? '180,77,255' : color === '#a855f7' ? '168,85,247' : '239,68,68'},`
  return (
    <button
      onClick={onClick}
      title={title}
      style={{
        ...
        background: active ? `${base}0.10)` : `rgba(7,4,26,0.0)`,
...
function Divider() {
  return <div style={{ width: 1, height: 18, background: 'rgba(180,77,255,0.1)', flexShrink: 0 }} />
}
...
function AskAction() {
  ...
  return (
    <>
      ...
      {open && (
        <div
          style={{ position:'fixed',inset:0,zIndex:10100,background:'rgba(7,4,26,0.80)',backdropFilter:'blur(12px)',WebkitBackdropFilter:'blur(12px)',display:'flex',alignItems:'flex-start',justifyContent:'center',paddingTop:'15vh' }}
          onClick={() => setOpen(false)}
        >
          <div
            style={{ width:'100%',maxWidth:600,margin:'0 16px',background:'rgba(9,6,22,0.98)',border:'1px solid rgba(168,85,247,0.22)',boxShadow:'0 32px 80px rgba(0,0,0,0.8)',borderRadius:6,overflow:'hidden' }}
          ...
          >
            ...
            <div style={{ padding:'12px 16px',borderBottom:'1px solid rgba(168,85,247,0.07)' }}>
              <input
                ...
                disabled={loading}
                style={{ width:'100%',background:'none',border:'none',outline:'none',fontFamily:'JetBrains Mono, monospace',fontSize:13,color:'#ede9ff',letterSpacing:'0.02em',opacity:loading?0.5:1 }}
              />
            </div>
...
function SnapshotAction({ activeModule }) {
  ...
  const snap = useCallback(async () => {
    ...
    try {
      ...
      // Footer bar
      ctx.fillStyle = 'rgba(7,4,26,0.96)'
...
export default function FloatingToolbar({ explainActive, onExplainToggle }) {
  ...
  return (
    <div style={{
      ...
      border: '1px solid rgba(180,77,255,0.10)',
      borderRadius: 5,
