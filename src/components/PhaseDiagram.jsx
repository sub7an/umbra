...
export default function PhaseDiagram() {
  ...
  const renderOverlay = useCallback(() => {
    ...
    if (cfg.boundary) {
      ctx.strokeStyle = 'rgba(180,77,255,0.7)'
      ctx.lineWidth = 1.5
      ctx.beginPath()
      let started = false
      for (let i = 0; i <= W; i++) {
  ...
  // ── Collapsed toggle ──────────────────────────────────────────────────────
  const accentColor = cfg ? 'rgba(180,77,255,0.6)' : 'rgba(255,255,255,0.15)'

  return (
    <div style={{ borderTop: '1px solid rgba(180,77,255,0.06)', background: 'rgba(1,6,12,0.97)', flexShrink: 0 }}>
      {/* Toggle bar */}
      <button
      ...
      {open && cfg && (
        <div style={{ padding:'0 14px 14px' }}>
          ...
          <p style={{ fontFamily:'JetBrains Mono,monospace', fontSize:9,
            letterSpacing:'.18em', color:'rgba(180,77,255,0.9)', margin:'0 0 3px' }}>
            {cfg.title}
          </p>
          ...
          {state !== 'done' && (
            <button
              ...
              style={{
                ...
                background: state === 'computing' ? 'rgba(180,77,255,0.06)' : 'rgba(180,77,255,0.09)',
                border:'1px solid rgba(180,77,255,0.25)',
                color:'rgba(180,77,255,0.9)', fontFamily:'JetBrains Mono,monospace',
            ...
            <div style={{ position:'relative', width:'100%', paddingBottom:'100%', background:'#010914', borderRadius:3, overflow:'hidden' }}>
              ...
              {state === 'computing' && (
                <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center',
                  ...
                  <span style={{ fontFamily:'JetBrains Mono,monospace', fontSize:9,
                    color:'rgba(180,77,255,0.7)', letterSpacing:'.2em',
              ...
              {tooltip && state === 'done' && (
                <div style={{
                  ...
                  border:'1px solid rgba(180,77,255,0.2)', borderRadius:2,
                  padding:'4px 7px', pointerEvents:'none',
                }}>
                  <p style={{ fontFamily:'JetBrains Mono,monospace', fontSize:8,
                    color:'rgba(180,77,255,0.9)', margin:0, lineHeight:1.8 }}>
            ...
            <div style={{ marginTop:8 }}>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                ...
                <button onClick={() => setState('idle')} style={{
                  ...
                  fontSize:7, color:'rgba(180,77,255,0.4)', cursor:'pointer', letterSpacing:'.1em',
                }}>
                  ↺ RECOMPUTE
                </button>
              </div>
              {cfg.boundary && (
                <p style={{ fontFamily:'JetBrains Mono,monospace', fontSize:7,
                  color:'rgba(180,77,255,0.5)', margin:'5px 0 0', letterSpacing:'.08em' }}>
                  ─── {cfg.boundary.label}
                </p>
              )}
            </div>
