...
function PreviewRelativity() {
  ...
  return (
    <group ref={groupRef}>
      ...
      {[...Array(9)].map((_, ri) => {
        ...
        return (
          <line key={`f${ri}`} geometry={geo}>
            <lineBasicMaterial color="#f59e0b" transparent opacity={0.25 + ri * 0.05} />
          </line>
        )
      })}
      {/* Past cone (inverted) */}
      {[...Array(9)].map((_, ri) => {
        ...
        return (
          <line key={`p${ri}`} geometry={geo}>
            <lineBasicMaterial color="#f59e0b" transparent opacity={0.12 + ri * 0.03} />
          </line>
        )
      })}
      {/* Time axis */}
      ...
      <pointLight position={[0, 0, 0]} color="#f59e0b" intensity={1} distance={3} />
    </group>
  )
}
