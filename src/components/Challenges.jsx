...
function VerifyPanel({ challenge, onSuccess, onDismiss }) {
  ...
  return (
    <div style={{
      ...
      background: 'rgba(10,7,4,0.96)', border: '1px solid rgba(245,158,11,0.14)',
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: 'Chakra Petch, sans-serif', fontSize: 11, fontWeight: 600, color: '#fef9ec', marginBottom: 3 }}>
            {challenge.title}
          </div>
          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 9, color: 'rgba(245,158,11,0.4)', letterSpacing: '0.08em' }}>
            {challenge.points} PTS
          </div>
        </div>
        <button onClick={onDismiss} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(245,158,11,0.3)', fontSize: 16, lineHeight: 1, padding: 0 }}>×</button>
      </div>
      ...
      <p style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 12, color: 'rgba(245,158,11,0.3)', lineHeight: 1.5, marginBottom: 12, letterSpacing: '-0.01em' }}>
        HINT: {challenge.hint}
      </p>
      ...
      <button
        ...
        onMouseLeave={e => { e.currentTarget.style.background = 'rgba(245,158,11,0.07)'; e.currentTarget.style.borderColor = 'rgba(245,158,11,0.25)' }}
      >
        VERIFY →
      </button>
    </div>
...
export default function Challenges() {
  ...
  return (
    <>
      ...
      {!activeModule && (
        <button
          ...
          style={{
            ...
            color: allDone ? '#10b981' : 'rgba(245,158,11,0.55)',
            background: 'rgba(8,6,4,0.75)',
            border: `1px solid ${allDone ? 'rgba(16,185,129,0.3)' : 'rgba(245,158,11,0.14)'}`,
            borderRadius: 4, cursor: 'pointer', backdropFilter: 'blur(8px)',
            WebkitBackdropFilter: 'blur(8px)',
            transition: 'color 0.15s, border-color 0.15s',
          }}
          onMouseEnter={e => { e.currentTarget.style.color = allDone ? '#10b981' : 'rgba(245,158,11,0.9)'; e.currentTarget.style.borderColor = allDone ? 'rgba(16,185,129,0.5)' : 'rgba(245,158,11,0.35)' }}
          onMouseLeave={e => { e.currentTarget.style.color = allDone ? '#10b981' : 'rgba(245,158,11,0.55)'; e.currentTarget.style.borderColor = allDone ? 'rgba(16,185,129,0.3)' : 'rgba(245,158,11,0.14)' }}
        >
      ...
      {open && (
        <div
          style={{ position:'fixed',inset:0,zIndex:10200,background:'rgba(8,6,4,0.85)',backdropFilter:'blur(12px)',WebkitBackdropFilter:'blur(12px)',display:'flex',alignItems:'flex-start',justifyContent:'center',paddingTop:'8vh',overflowY:'auto' }}
          onClick={() => setOpen(false)}
        >
          <div
            style={{ width:'100%',maxWidth:640,margin:'0 16px 32px',background:'rgba(10,7,4,0.98)',border:'1px solid rgba(245,158,11,0.14)',borderRadius:6,overflow:'hidden',boxShadow:'0 32px 80px rgba(0,0,0,0.8)' }}
            onClick={e => e.stopPropagation()}
          >
            {/* Header */}
            <div style={{ padding:'16px 20px',borderBottom:'1px solid rgba(245,158,11,0.08)',display:'flex',alignItems:'center',gap:12 }}>
              <div>
                <div style={{ fontFamily:'Chakra Petch, sans-serif',fontSize:16,fontWeight:700,color:'#fef9ec',letterSpacing:'0.06em' }}>DAILY CHALLENGES</div>
                <div style={{ fontFamily:'JetBrains Mono, monospace',fontSize:10,color:'rgba(245,158,11,0.55)',letterSpacing:'0.14em',marginTop:2 }}>{TODAY} · RESETS MIDNIGHT</div>
              </div>
              <div style={{ marginLeft:'auto',display:'flex',alignItems:'center',gap:16 }}>
                ...
                <div style={{ textAlign:'center' }}>
                  <div style={{ fontFamily:'Chakra Petch, sans-serif',fontSize:18,fontWeight:700,color:'#f59e0b' }}>{totalPts}</div>
                  <div style={{ fontFamily:'JetBrains Mono, monospace',fontSize:10,color:'rgba(245,158,11,0.60)',letterSpacing:'0.12em' }}>TODAY PTS</div>
                </div>
              </div>
            </div>

            {/* Progress bar */}
            <div style={{ height:2,background:'rgba(245,158,11,0.06)' }}>
              <div style={{ height:'100%',background:'rgba(245,158,11,0.45)',width:`${(doneToday/CHALLENGES.length)*100}%`,transition:'width 0.4s' }} />
            </div>
                  ...
                  >
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:3 }}>
                        <span style={{ fontFamily:'Chakra Petch, sans-serif',fontSize:12,fontWeight:600,color:done?'rgba(16,185,129,0.7)':'#fef9ec' }}>{ch.title}</span>
            ...
            {/* Footer */}
            <div style={{ padding:'12px 20px',borderTop:'1px solid rgba(245,158,11,0.06)',display:'flex',alignItems:'center',gap:12 }}>
              <span style={{ fontFamily:"'Inter', system-ui, sans-serif",fontSize:12,color:'rgba(245,158,11,0.50)',letterSpacing:'-0.01em' }}>
                Complete all 5 daily challenges to extend your streak
              </span>
              <kbd style={{ marginLeft:'auto',fontFamily:'JetBrains Mono, monospace',fontSize:10,color:'rgba(245,158,11,0.45)',border:'1px solid rgba(245,158,11,0.20)',borderRadius:3,padding:'2px 5px' }}>SHIFT+C</kbd>
            </div>
          </div>
        </div>
      )}
