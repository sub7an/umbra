...
function EventPoint({ position, onDrag, region }) {
  ...
  const colorMap = { timelike: '#b44dff', spacelike: '#e040fb', lightlike: '#f59e0b' }
  const color = colorMap[region] ?? '#ede9ff'
...
export default function LightCone() {
  ...
  const regionColor = { timelike: '#b44dff', spacelike: '#e040fb', lightlike: '#f59e0b' }[region]

  return (
    <group>
      {/* ── Solid transparent cone volumes ── */}
      {/* Future cone: ConeGeometry apex at +h/2 local → flip with rotX=PI + position at h/2 */}
      <SolidCone posY={height / 2} rotX={Math.PI} h={height} color="#b44dff" opacity={0.06} />
      {/* Past cone: apex at +h/2 local → position at -h/2 gives apex at origin */}
      <SolidCone posY={-height / 2} rotX={0} h={height} color="#6d28d9" opacity={0.04} />
      ...
      <PulseRings height={height} color="#b44dff" dir={1} speed={0.14} />
      <PulseRings height={height} color="#6d28d9" dir={-1} speed={0.11} />

      {/* ── Wireframe cone lines ── */}
      <ConeLines apex={0} height={height} radius={height} color="#b44dff" opacity={0.6} />
      <ConeLines apex={0} height={-height} radius={height} color="#6d28d9" opacity={0.35} />
      ...
      <Html position={[0, 2.4, 0]} center style={{ pointerEvents: 'none' }}>
        <span style={{ fontFamily: 'Chakra Petch,sans-serif', fontSize: 10, color: '#9333ea', letterSpacing: '0.12em', whiteSpace: 'nowrap' }}>FUTURE</span>
      </Html>
      <Html position={[0, -2.4, 0]} center style={{ pointerEvents: 'none' }}>
        <span style={{ fontFamily: 'Chakra Petch,sans-serif', fontSize: 10, color: '#6d28d9', letterSpacing: '0.12em', whiteSpace: 'nowrap' }}>PAST</span>
      </Html>
