...
function Galaxy() {
  ...
  return (
    <group ref={groupRef} position={GALAXY_CENTER}>
      ...
      <pointLight color="#5e6ad2" intensity={0.6} distance={1.8} />
      <mesh>
        <sphereGeometry args={[0.15, 12, 12]} />
        <meshStandardMaterial color="#5e6ad2" emissive="#5e6ad2" emissiveIntensity={1.0} transparent opacity={0.5} />
      </mesh>
    </group>
  )
}
  ...
  return (
    <>
      ...
      <lineSegments geometry={arrowGeo}>
        <lineBasicMaterial color="#5e6ad2" transparent opacity={0.85} depthWrite={false} />
      </lineSegments>
    </>
  )
}
  ...
  return (
    <div style={{ width: G_W, background: 'rgba(5,9,12,0.95)', border: '1px solid rgba(245,158,11,0.28)', borderRadius: 6, overflow: 'hidden', userSelect: 'none' }}>
      ...
      <svg width={G_W} height={G_H}>
        <rect x={PAD.l} y={PAD.t} width={pw} height={ph} fill="rgba(17,17,19,0.6)" />
        ...
        <path d={obsPath} fill="none" stroke="#5e6ad2" strokeWidth={2} />

        <circle cx={rx} cy={ky} r={4.5} fill="#f59e0b" opacity={0.9} />
        <circle cx={rx} cy={oy} r={4.5} fill="#5e6ad2" />
        ...
        <line x1={PAD.l + 4} y1={PAD.t + 8} x2={PAD.l + 18} y2={PAD.t + 8} stroke="#5e6ad2" strokeWidth={2} />
        <text x={PAD.l + 22} y={PAD.t + 11} fill="#5e6ad2" fontSize={8} fontFamily="JetBrains Mono, monospace">measured</text>
