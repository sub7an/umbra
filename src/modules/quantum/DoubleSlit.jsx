...
function SourceWaves() {
  ...
  const mats = useMemo(() => Array.from({ length: WAVE_N }, () =>
    new THREE.LineBasicMaterial({ color: new THREE.Color('#b44dff'), transparent: true, opacity: 0 })
  ), [])
...
function SlitWaves({ measured }) {
  const SEGS = 36
  const color = measured ? '#f59e0b' : '#b44dff'
...
function Barrier({ measured }) {
  ...
  const slitGlowColor = measured ? '#f59e0b' : '#b44dff'
...
function ParticleCloud({ measured, lambda }) {
  ...
  const color = measured ? '#f59e0b' : '#b44dff'
  const emissive = measured ? '#f59e0b' : '#b44dff'
  ...
  return (
    <group>
      ...
      <mesh position={[SOURCE_X, 0, 0]}>
        ...
        <meshStandardMaterial color="#b44dff" emissive="#b44dff" emissiveIntensity={1.5} />
      </mesh>
      <pointLight position={[SOURCE_X, 0, 0]} color="#b44dff" intensity={0.8} distance={2} />
      ...
      <Html position={[SCREEN_X + 1.0, 0, 0]} center style={{ pointerEvents: 'none', textAlign: 'center' }}>
        <div style={{
          ...
          fontSize: 10,
          color: measured ? '#f59e0b' : '#b44dff',
          textShadow: measured ? '0 0 6px rgba(245,158,11,0.5)' : '0 0 6px rgba(180,77,255,0.5)',
          whiteSpace: 'nowrap',
        }}>
