...
function SourceWaves() {
  ...
  const mats = useMemo(() => Array.from({ length: WAVE_N }, () =>
    new THREE.LineBasicMaterial({ color: new THREE.Color('#5e6ad2'), transparent: true, opacity: 0 })
  ), [])
...
function SlitWaves({ measured }) {
  const SEGS = 36
  const color = measured ? '#f59e0b' : '#5e6ad2'
...
function Barrier({ measured }) {
  ...
  const slitGlowColor = measured ? '#f59e0b' : '#5e6ad2'
...
function ParticleCloud({ measured, lambda }) {
  ...
  const color = measured ? '#f59e0b' : '#5e6ad2'
  const emissive = measured ? '#f59e0b' : '#5e6ad2'
  ...
  return (
    <group>
      ...
      <mesh position={[SOURCE_X, 0, 0]}>
        ...
        <meshStandardMaterial color="#5e6ad2" emissive="#5e6ad2" emissiveIntensity={1.5} />
      </mesh>
      <pointLight position={[SOURCE_X, 0, 0]} color="#5e6ad2" intensity={0.8} distance={2} />
