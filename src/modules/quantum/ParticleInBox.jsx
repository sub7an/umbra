...
function WaveCurve({ n, En, vizMode }) {
  ...
  useFrame(({ clock }) => {
    ...
    else if (vizMode === 'real') mat.color.set('#5e6ad2')
    ...
    <>
      {nodes.map((x, i) => (
        <mesh key={i} position={[x, 0, 0]}>
          ...
          <meshStandardMaterial color="#222226" emissive="#5e6ad2" emissiveIntensity={0.6} />
        </mesh>
      ))}
    </>
  )
...
function WallGlow({ x }) {
  return (
    <mesh position={[x, 0, -0.15]}>
      ...
      <meshBasicMaterial color={new THREE.Color('#5e6ad2')} transparent opacity={0.06} blending={THREE.AdditiveBlending} depthWrite={false} />
    </mesh>
  )
}
  ...
  return (
    <group position={[0, 0.2, 0]}>
      ...
      <mesh position={[BOX_LEFT, 0, 0]}>
        ...
        <meshStandardMaterial color="#5e6ad2" emissive="#5e6ad2" emissiveIntensity={0.7} roughness={0.3} />
      </mesh>
      <mesh position={[BOX_RIGHT, 0, 0]}>
        <boxGeometry args={[0.05, WALL_H, 0.05]} />
        <meshStandardMaterial color="#5e6ad2" emissive="#5e6ad2" emissiveIntensity={0.7} roughness={0.3} />
      </mesh>
      <pointLight position={[BOX_LEFT, 0, 0.3]} color="#5e6ad2" intensity={0.5} distance={1.5} />
      <pointLight position={[BOX_RIGHT, 0, 0.3]} color="#5e6ad2" intensity={0.5} distance={1.5} />
